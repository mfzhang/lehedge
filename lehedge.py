...
#    h['post_window_duration'] = h['dtmil'].shift(-best_window_size) - h['dtmil']
    ...
    h['post_window_duration_seconds'] = h['post_window_duration'] / np.timedelta64(1, 's')

    print "Filtering out rows part of everlasting forward windows (> 2 hours)"
    a_long_enough_period_to_identify_closed_markets = 7200
    h = h[h['post_window_duration_seconds'] < a_long_enough_period_to_identify_closed_markets]

    print "Filtering out rows with null profit and casting profit to int"
    h['post_window_profit'] = h['post_window_profit'][h['post_window_profit'].notnull()].round().astype('int')

    print "Creating backward windows with twice the length of the forward window for each row"
    pre_window_size = best_window_size*2
    for lag in range(0, pre_window_size):
        colname = 'r' + str(lag)
        h[colname] = h['rate'].shift(lag)

    print "Filtering out rows with incomplete backward window"
    d = h[h['r' + str(pre_window_size - 1)].notnull()]

    return d

d = load_historical_data("./$AUDUSD.Ask.txt",10000)

bins = {(int(d['post_window_profit'].min()), -10.0): -1,
        (-9.0, 9.0): 0,
        (10.0, int(d['post_window_profit'].max())): 1}
...
for (r, label) in bins.iteritems():
    ...
    mask = ((d['post_window_profit'] >= r[0]) & (d['post_window_profit'] <= r[1]))
    df = d[mask]
